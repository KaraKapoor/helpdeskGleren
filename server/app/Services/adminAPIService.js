...
exports.getProjectByName = async (projectName, tenantId) => {
  return await project.findOne({
    where: { [Op.and]: [{ name: projectName }, { tenant_id: tenantId }] },
  });
};
...
exports.getStatusByName = async (statusName, tenantId) => {
  return await status.findOne({
    where: { [Op.and]: [{ name: statusName }, { tenant_id: tenantId }] },
  });
};
...
exports.bugReportEmail = async (bugDescription, tenantId) => {
  ...
  bugReportTemplate = bugReportTemplate.replace(
    "{bugDescription}",
    bugDescription
  );

  await emailAPIService.sendEmail(
    coreSettingsConstants.SUPPORT_EMAIL,
    emailTemplates.BUG_REPORT_SUBJECT,
    null,
    null,
    null,
    bugReportTemplate
...
exports.getDepartmentByName = async (departmentName, tenantId) => {
  return await department.findOne({
    where: { [Op.and]: [{ name: departmentName }, { tenant_id: tenantId }] },
  });
};
...
exports.masterDropdownData = async (tenantId, currentUserId) => {
  ...
  const usersData = await user.findAll({
    where: {
      [Op.and]: [
        { tenant_id: tenantId },
        { role: { [Op.in]: ["admin", "agent", "teamLead"] } },
      ],
    },
  });
  response["agents"] = usersData;
  const projectsData = await project.findAll({
    where: { [Op.and]: [{ tenant_id: tenantId }, { is_active: true }] },
  });
  response["projects"] = projectsData;

  let currenUserProjectsQuery = queries.GET_LOGGED_IN_USER_PROJECTS;
  currenUserProjectsQuery = currenUserProjectsQuery.replace(
    /:id/g,
    currentUserId
  );
  currenUserProjectsQuery = currenUserProjectsQuery.replace(
  ...
  const currenUserProjects = await this.executeRawSelectQuery(
    currenUserProjectsQuery
  );
  response["currentUserProjects"] = currenUserProjects;
  const onlyUsersData = await user.findAll({
    where: {
      [Op.and]: [
        { tenant_id: tenantId },
        { role: { [Op.notIn]: ["admin", "agent", "teamLead"] } },
      ],
    },
  });
  ...
  const activeStatus = await status.findAll({
    where: { [Op.and]: [{ tenant_id: tenantId }, { is_active: true }] },
  });
...
exports.getEscalationByDepartmentId = async (departmentId, tenantId) => {
  return await escalations.findOne({
    where: {
      [Op.and]: [{ department_id: departmentId }, { tenant_id: tenantId }],
    },
  });
};
exports.createEscalations = async (
  departmentId,
  l1Id,
  l2Id,
  l3Id,
  ...
  const createdEscalations = await this.getEscalationByDepartmentId(
    departmentId,
    tenantId
  );
  response = {
...
exports.getEscalationById = async (id, tenantId) => {
  let response = null;
  const data = await escalations.findOne({
    where: { [Op.and]: [{ id: id }, { tenant_id: tenantId }] },
  });
  if (data !== null) {
...
exports.getTeamByName = async (teamName, tenantId) => {
  return await team.findOne({
    where: { [Op.and]: [{ name: teamName }, { tenant_id: tenantId }] },
  });
};
exports.createTeam = async (
  ...
  if (
    ...
    await teamAgentAssociation.destroy({
      where: { [Op.and]: [{ team_id: id }, { tenant_id: tenantId }] },
    });
    //Insert Leads
    for (let i of leads) {
      const obj = {
...
exports.getTeamById = async (id, tenantId) => {
  ...
  let leadsArray = [];
  let usersArray = [];
  const teamResp = await team.findOne({
    where: { [Op.and]: [{ id: id }, { tenant_id: tenantId }] },
  });
  if (teamResp !== null) {
    response = teamResp.dataValues;

    const associationResp = await teamAgentAssociation.findAll({
      where: { [Op.and]: [{ tenant_id: tenantId }, { team_id: id }] },
    });
