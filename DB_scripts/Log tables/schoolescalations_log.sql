CREATE TABLE `schoolescalations_log` (
   `LogId` INT NOT NULL AUTO_INCREMENT,
  `idschoolescalation` int NOT NULL,
...
-- Insert Operation
DROP TRIGGER if exists schoolescalations_insert_log;
DELIMITER $$
CREATE TRIGGER schoolescalations_insert_log AFTER INSERT ON schoolescalations
FOR EACH ROW
BEGIN
  INSERT INTO schoolescalations_log (idschoolescalation,branch,department,module,state,district,agm,blankcol,l1name,l1mobile,l1email,l2name,l2mobile,l2email,l3name,l3mobile,l3email,l4name,l4mobile,l4email,l5name,l5mobile,l5email,hodname,hodmobile,hodemail,createdAt,updatedAt,operationName,operationTime)
 VALUES(NEW.idschoolescalation,NEW.branch,NEW.department,NEW.module,NEW.state,NEW.district,NEW.agm,NEW.blankcol,NEW.l1name,NEW.l1mobile,NEW.l1email,NEW.l2name,NEW.l2mobile,NEW.l2email,NEW.l3name,NEW.l3mobile,NEW.l3email,NEW.l4name,NEW.l4mobile,NEW.l4email,NEW.l5name,NEW.l5mobile,NEW.l5email,NEW.hodname,NEW.hodmobile,NEW.hodemail,NEW.createdAt,NEW.updatedAt, 'insert',NOW());
END$$
DELIMITER ;

-- Update Operation
DROP TRIGGER if exists schoolescalations_update_log;
DELIMITER $$
CREATE TRIGGER schoolescalations_update_log AFTER UPDATE ON schoolescalations
FOR EACH ROW
BEGIN
  INSERT INTO schoolescalations_log (idschoolescalation,branch,department,module,state,district,agm,blankcol,l1name,l1mobile,l1email,l2name,l2mobile,l2email,l3name,l3mobile,l3email,l4name,l4mobile,l4email,l5name,l5mobile,l5email,hodname,hodmobile,hodemail,createdAt,updatedAt,operationName,operationTime)
...
-- Delete Operation
DROP TRIGGER if exists schoolescalations_delete_log;
DELIMITER $$
CREATE TRIGGER schoolescalations_delete_log AFTER DELETE ON schoolescalations
FOR EACH ROW
BEGIN
  INSERT INTO schoolescalations_log (idschoolescalation,branch,department,module,state,district,agm,blankcol,l1name,l1mobile,l1email,l2name,l2mobile,l2email,l3name,l3mobile,l3email,l4name,l4mobile,l4email,l5name,l5mobile,l5email,hodname,hodmobile,hodemail,createdAt,updatedAt,operationName,operationTime)
  VALUES(OLD.idschoolescalation,OLD.branch,OLD.department,OLD.module,OLD.state,OLD.district,OLD.agm,OLD.blankcol,OLD.l1name,OLD.l1mobile,OLD.l1email,OLD.l2name,OLD.l2mobile,OLD.l2email,OLD.l3name,OLD.l3mobile,OLD.l3email,OLD.l4name,OLD.l4mobile,OLD.l4email,OLD.l5name,OLD.l5mobile,OLD.l5email,OLD.hodname,OLD.hodmobile,OLD.hodemail,OLD.createdAt,OLD.updatedAt, 'delete',NOW());
END$$
DELIMITER ;
