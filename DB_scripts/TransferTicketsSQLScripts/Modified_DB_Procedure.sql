...
DECLARE ticketId6 INTEGER DEFAULT 0;
	 ...
	 DEClARE curTicketId 
 		CURSOR FOR 
 			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes) MINUTE) < now() and level1SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed') and (isTicketWronglyAssigned=0 or isTicketWronglyAssigned is null);
        
    DEClARE curTicketIdSLA2 
		CURSOR FOR 
			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes*2) MINUTE) < now() and level1SlaTriggered is not null and level2SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed') and (isTicketWronglyAssigned=0 or isTicketWronglyAssigned is null);
	
    DEClARE curTicketIdSLA3
		CURSOR FOR 
			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes *3) MINUTE) < now() and level1SlaTriggered is not null and level2SlaTriggered is not null and level3SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed') and (isTicketWronglyAssigned=0 or isTicketWronglyAssigned is null);
	
    DEClARE curTicketIdSLA4
		CURSOR FOR 
			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes *4) MINUTE) < now() and level1SlaTriggered is not null and level2SlaTriggered is not null and level3SlaTriggered is not null and level4SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed') and (isTicketWronglyAssigned=0 or isTicketWronglyAssigned is null);
    
    DEClARE curTicketIdSLA5
		CURSOR FOR 
			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes *5) MINUTE) < now() and level1SlaTriggered is not null and level2SlaTriggered is not null and level3SlaTriggered is not null and level4SlaTriggered is not null and level5SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed') and (isTicketWronglyAssigned=0 or isTicketWronglyAssigned is null);
    
    DEClARE curTicketIdSLA6
		CURSOR FOR 
			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes *6) MINUTE) < now() and level1SlaTriggered is not null and level2SlaTriggered is not null and level3SlaTriggered is not null and level4SlaTriggered is not null and level5SlaTriggered is not null and hodSlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed') and (isTicketWronglyAssigned=0 or isTicketWronglyAssigned is null);
	...
	getTicketId: LOOP
		...
		UPDATE `narayanaminiapp`.`tickets` SET `level1SlaTriggered` =1,`isTicketOverdue`="Yes" WHERE `id` = ticketId1;
	...
	getTicketId2: LOOP
		...
		UPDATE `narayanaminiapp`.`tickets` SET `level2SlaTriggered` =1,`isTicketOverdue`="Yes" WHERE `id` = ticketId2;
	...
	getTicketId3: LOOP
		...
		UPDATE `narayanaminiapp`.`tickets` SET `level3SlaTriggered` =1,`isTicketOverdue`="Yes" WHERE `id` = ticketId3;
	...
	getTicketId4: LOOP
		...
		UPDATE `narayanaminiapp`.`tickets` SET `level4SlaTriggered` =1,`isTicketOverdue`="Yes" WHERE `id` = ticketId4;
	...
	getTicketId5: LOOP
		...
		UPDATE `narayanaminiapp`.`tickets` SET `level5SlaTriggered` =1,`isTicketOverdue`="Yes" WHERE `id` = ticketId5;
	...
	getTicketId6: LOOP
		...
		UPDATE `narayanaminiapp`.`tickets` SET `hodSlaTriggered` =1,`isTicketOverdue`="Yes" WHERE `id` = ticketId6;
