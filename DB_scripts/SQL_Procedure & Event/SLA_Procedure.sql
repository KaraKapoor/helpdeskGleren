...
DECLARE ticketId6 INTEGER DEFAULT 0;
	 ...
	 DEClARE curTicketId 
 		CURSOR FOR 
 			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes) MINUTE) < now() and level1SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed');
        
    DEClARE curTicketIdSLA2 
		CURSOR FOR 
			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes*2) MINUTE) < now() and level1SlaTriggered is not null and level2SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed');
	
    DEClARE curTicketIdSLA3
		CURSOR FOR 
			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes *3) MINUTE) < now() and level1SlaTriggered is not null and level2SlaTriggered is not null and level3SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed');
	
    DEClARE curTicketIdSLA4
		CURSOR FOR 
			SELECT id FROM narayanaminiapp.tickets where ADDDATE(createdAt, INTERVAL (slaPlanInMinutes *4) MINUTE) < now() and level1SlaTriggered is not null and level2SlaTriggered is not null and level3SlaTriggered is not null and level4SlaTriggered is null and ticketStatus NOT IN ('Resolved','Closed');
