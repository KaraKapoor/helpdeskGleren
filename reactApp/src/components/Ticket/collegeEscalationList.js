...
class collegeEscalationList extends Component {
    ...
    constructor(props) {
        ...
        this.columns = [
            { title: "College Escalation Id", field: "idcollegeescalation", render: rowData =><span><i class="fa fa-users" aria-hidden="true"></i>
            &nbsp;&nbsp;<Link onClick={this.savePageLocation("/collegeList")}to={`/addCollegeEscalation/id:${rowData.idcollegeescalation}`}>{rowData.idcollegeescalation}</Link> 
            </span> },
    ...
    clickAdd() {
        this.props.history.push("/addCollegeEscalation");
    }
    async remoteData(query, status) {
        ...
        await TicketDataService.getAllCollegeEscalationList(query.pageSize, query.page, query.search, orderDirection, orderBy)
            .then(async (response) => {
                //let result = response.data;
                for (var i of response.data.collegeEscalationList) {
                    i.idcollegeescalation = i.idcollegeescalation;
                    i.branch = i.branch;
                    i.department = i.department;
                    i.module=i.module;
                    // i.helpdeskRole=i.helpdeskRole;
                }
                data = response.data.collegeEscalationList;
    ...
    async exportData() {
        ...
        this.state.exportDataDownloadLink = url.baseURL + `/api/dataExport/downloadCollegeEscalation`;
        ...
        return (
            <div id="test">
                <div style={{ marginTop: "20px" }}>
                    <h5 className="formHeading">College Escalation</h5>
                    <div>
...
export default collegeEscalationList;